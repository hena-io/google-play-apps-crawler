...
let COLLECTED_GOOGLE_PLAY_APP_LIST: object = {};
let COLLECTED_GOOGLE_PLAY_APP_DETAILS: object = {};

let NUM_TOTAL_COLLECTED:number = 0;

let CollectGooglePlayAppList_Internal = async (country, collection, category) => {
...
// 구글 앱 목록 긁어오기
let CollectGooglePlayAppList = async (countries, collections, categories) => {
	...
	let totalCount: number = countries.length * collections.length * categories.length;
	...
	for (var it_country in countries) {
		for (var it_collection in collections) {
			...
			for (var it_category in categories) {
				...
				let apps = await CollectGooglePlayAppList_Internal(country, collection, category);
...
let CollectGooglePlayAppDetailsByAppList = async (apps, exportToExcel, filename) => {
...
let run = async () => {
	mkdirSync('output');
	let appList = await CollectGooglePlayAppList(COUNTRY, COLLECTIONS, CATEGORIES);
	let appDetails = await CollectGooglePlayAppDetailsByAppList(appList, true, './output/GooglePlayApps');
